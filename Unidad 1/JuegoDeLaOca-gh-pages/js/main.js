...
//Crear los objetos con las imagenes.
	var fondo = {
		imagenURL: "img/juego_de_la_oca_tablero.png"
	};
...
function lanzarDado(){
	...
	window.setTimeout("moverFichas()",1500);
}	
//Se mueven las fichas al tirada del dado.
...
function comprobarCasilla(casilla){
	...
	//casillas finales para llegar a la meta
	else if(casilla == 64){
		casilla_64();
	}
	else if(casilla == 65){
...
function cambiarCubilete(){
	...
	var dadoUrl;
	if(turnoRoja === true){
		dadoUrl = "url(img/cubilete-verde.png)";
...
function cambiarDado(){
	dado = document.getElementById("dadoEnJuego");
	if(turnoRoja === true){
		var dadoUrl = "url(img/dadoVerde_"+movimiento+".png)";
		dado.style.backgroundImage=dadoUrl;
	}
	else if(turnoAmarilla === true){
...
function seleccionarFicha(color){
	...
	if(color == "rojo"){
		colorJugador = color;
		juegasCon.innerHTML = "Juegas con el REY"
		 
	}
	else if(color == "amarillo"){
		colorJugador = color;
		juegasCon.innerHTML = "Juegas con la REYNA"
